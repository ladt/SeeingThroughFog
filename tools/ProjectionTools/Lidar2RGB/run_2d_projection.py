...
if __name__ == '__main__':
    ...
    files = os.listdir(os.path.join(args.root, 'cam_stereo_left_lut'))
    interesting_samples = [ Path(file).stem for file in files ]
    ...
    for interesting_sample in interesting_samples:
        ...
        img_pth = os.path.join(args.root, 'cam_stereo_left_lut', interesting_sample + '.png')
        out_dir = os.path.join(args.root, 'lidar_cam_project')
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        out_pth = os.path.join(out_dir, interesting_sample + '.png')
        lib.settings.grid_size = 2
